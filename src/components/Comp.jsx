...
const Comp = () => {
  ...
  return (
    ...
    <>
      ...
      <div>
        <input
          type="text" className="m-5 border border-black"
          placeholder="Search by name"
          value={query}
          onChange={(e) => setQuery(e.target.value)}
        />
        <button className="bg-green-800 text-white border border-black m-5" onClick={handleSearch}>Search</button>
      </div>
        ...
        <div className="bg-blue-200 h-96 relative">
          ...
          <div className="">
            <ul>
              {results.map((result) => (
                <li key={result.id}>
                  <strong>{result.name}</strong>: {result.contact}
                </li>
              ))}
